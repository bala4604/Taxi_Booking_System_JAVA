...
class taxiBook extends Thread
{
    ...
    {//get details for booking and allot taxi
	...
	for(taxi t:taxies)
	{
            ...
            {//assign a taxi when pickup point and current point of the taxi is matched
	...
	if(rej==1)
	{//means no taxi is free, so reject the booking
	...
	for(taxi t1:taxies)
	{//get nearest taxies in another array list
            if(t1.isFree==true)
            {
	...
	{//if we have only one nearest taxi
	...
	else
	{
		...
		int mLoc=0;
		for(int i=1;i<nearestTaxies.size();i++)
		{//get minimum earning taxi from minimum distance taxies
